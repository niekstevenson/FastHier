...
2.  The user supplies or accepts a group model. For now the group model is diagonal Gaussian: `theta = (mu_1, ..., mu_d, log_sigma2_1, ..., log_sigma2_d)`.
3.  The framework builds certified local marginal likelihood surfaces `m_i(theta) = integral L_i(alpha) p(alpha | theta) d alpha`.
4.  The framework runs outer SMC on the frozen product `p(theta) prod_i m_i(theta)`.
...
-   The root chart target is `pi_i,root(alpha) proportional to L_i(alpha) p(alpha | theta_root)`.
...
-   Each chart target is `pi_i,a(alpha) proportional to L_i(alpha) p(alpha | theta_a)`.
...
-   Samples the group-level posterior: `pi(theta) proportional to p(theta) prod_i m_i(theta)`.
